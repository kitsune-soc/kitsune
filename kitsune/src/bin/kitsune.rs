...
use kitsune::{
    activitypub::Fetcher, config::Configuration, db, http, job, search::SearchService,
...
async fn main() {
    ...
    let search_service = SearchService::new(&config.search_index_server, &config.search_servers)
        .await
