...
impl Issuer for OAuthIssuer {
    ...
    async fn issue(&mut self, grant: Grant) -> Result<IssuedToken, ()> {
        let result: Result<_> = attempt! { async
            ...
            let (access_token, refresh_token) = with_transaction!(self.db_pool, |tx| {
        ...
        result.map_err(|error| debug!(?error, "failed to issue token"))
    }
    ...
    async fn refresh(&mut self, refresh_token: &str, grant: Grant) -> Result<RefreshedToken, ()> {
        ...
        result.map_err(|error| debug!(?error, "failed to refresh token"))
    }
    ...
    async fn recover_token(&mut self, access_token: &str) -> Result<Option<Grant>, ()> {
        let result: Result<_> = attempt! { async
            ...
            oauth_data.map(|(access_token, app)| {
        ...
        result.map_err(|error| debug!(?error, "failed to recover token grant"))
    }
    ...
    async fn recover_refresh(&mut self, refresh_token: &str) -> Result<Option<Grant>, ()> {
        ...
        result.map_err(|error| debug!(?error, "failed to recover refresh grant"))
    }
}
