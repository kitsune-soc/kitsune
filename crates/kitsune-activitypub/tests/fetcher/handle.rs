    ...
    match req.uri().path_and_query().unwrap().as_str() {
        ...
        "/@0x0/109501674056556919" => {
            let body = include_str!(
                "../../../../test-fixtures/activitypub/corteximplant.com_109501674056556919.json"
            );
            Ok::<_, Infallible>(build_ap_response(body))
        }
        "/users/0x0/statuses/109501659207519785" => {
            let body = include_str!(
                "../../../../test-fixtures/activitypub/corteximplant.com_109501659207519785.json"
            );
            Ok::<_, Infallible>(build_ap_response(body))
        }
        "/emojis/7952" => {
            let body = include_str!(
                "../../../../test-fixtures/activitypub/corteximplant.com_emoji_7952.json"
            );
            Ok::<_, Infallible>(build_ap_response(body))
        }
        "/emojis/8933" => {
            let body = include_str!(
                "../../../../test-fixtures/activitypub/corteximplant.com_emoji_8933.json"
            );
            Ok::<_, Infallible>(build_ap_response(body))
        }
